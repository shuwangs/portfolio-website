export const GET_ALL_BLOGS = `
    SELECT p.id, p.slug, p.title, p.summary, p.created_at, cats.name AS category_name, ARRAY_AGG (tags.name) AS tags
    FROM posts p
    ...
    GROUP BY p.id, p.slug, p.title, p.summary, p.created_at, category_name
